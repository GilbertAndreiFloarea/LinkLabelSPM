...
public class LinkLabel: UILabel {
    ...
    public override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        ...
        // checking if the point is actually inside the text bounds
        let lineTapped = Int(ceil(locationOfTouchInLabel.y / font.lineHeight)) - 1
        let rightestPointInLineTapped = CGPoint(x: bounds.size.width, y: font.lineHeight * CGFloat(lineTapped))
        let charsInLineTapped = layoutManager.characterIndex(
            for: rightestPointInLineTapped,
            in: customTextContainer,
            fractionOfDistanceBetweenInsertionPoints: nil
        )
